...
assert(
    pkg.name === "codecov-management-skill",
    "package name must be codecov-management-skill"
);
assert(
    pkg.repository?.url ===
        "git+https://github.com/Nick2bad4u/Codecov-Skill.git",
    "repository.url must exactly match the GitHub repository for npm trusted publishing"
);
assert(
    skill.path === `skills/${skill.name}`,
    "codexSkill.path must point at skills/<skill-name>"
);
