...
2. Ensure Python 3.14 is available for the strict validation setup.
...
- **Do not** pass Codecov tokens as CLI literals.
- Use environment variables (`CODECOV_TOKEN`, `CODECOV_API_TOKEN`, or `--token-env`).
- Prefer `--dry-run` for non-GET fallback API calls in docs/examples.
